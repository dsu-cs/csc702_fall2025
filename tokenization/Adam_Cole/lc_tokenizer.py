...
if not Path("saved_tokenizers/lovecraft-WPvocab.json").exists():
    WPTokenizer = Tokenizer(WordPiece())
    with open("../../words_to_emb/Ashar_Adam/data/lovecraft.txt", "r", encoding="utf-8") as f:
            WPTokenizer.train_from_iterator(f)
    WPTokenizer.save("saved_tokenizers/lovecraft-WPvocab.json")
else:
    ...
    WPTokenizer = Tokenizer.from_file("saved_tokenizers/lovecraft-vocab.json")
...
enc = WPTokenizer.encode("It is a truth universally acknowledged, that a single man in possession of a good fortune, must be in want of a wife.")
print(enc.n_sequences, enc.ids, enc.tokens, end="\n\n")

# Something slightly longer, from one of H.P.'s penpals.
enc = WPTokenizer.encode("Hither came Conan, the Cimmerian, black-haired, sullen-eyed, sword in hand, a thief, a reaver, a slayer, with gigantic melancholies and gigantic mirth, to tread the jeweled thrones of the Earth under his sandalled feet")
print(enc.n_sequences, enc.ids, enc.tokens, end="\n\n")

# Longer. Orwell. (_Down and Out in Paris and London_)
enc = WPTokenizer.encode("""
It is not a figure of 
speech, it is a mere statement of fact to say that a French 
cook will spit in the soup— that is, if he is not going to 
drink it himself. He is an artist, but his art is not cleanliness. 
