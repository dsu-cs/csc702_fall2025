...
Moving on, this class will basically create two embeddings (english and german) and then compares the similarities between the source (input), and target embeddings (transpose of the other vocabularies vocab). Moreover, then, we basically rank the similarity scores and report the k (hardcoded to 5 here) most similar scores (and more importantly the accompanying words).
