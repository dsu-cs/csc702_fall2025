...
def objective(trial, base_args):
    ...
    cfg = RFConfig(
        ...
        n_jobs=base_args.n_jobs,
        save_path=None,
    )

    if cfg.dataset == "adult":
        X, y, pipe = build_pipeline_adult(cfg)
        X_train, y_train, X_val, y_val, _, _ = split_train_val_test(X, y, cfg)
        pipe.fit(X_train, y_train)
...
def main():
    args = parse_args()
    if hasattr(args, "optuna") and args.optuna:
        import optuna
...
if __name__ == "__main__":
    import sys
    # Add --optuna flag to CLI
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--optuna", action="store_true", help="Run Bayesian optimization with Optuna")
