...
def objective(trial, base_args):
    ...
    cfg = TrainConfig(
        ...
        save_path=None,  # Don't save during optimization
    )

    # Build model with hidden_dim if MLP
    if model_type == "mlp":
        model = MLP(hidden_dim=hidden_dim, dropout=dropout).to(get_device())
    else:
        model = SmallCNN(dropout=dropout).to(get_device())
    ...
    for epoch in range(epochs):
        train_one_epoch(model, train_loader, criterion, optimizer, get_device())
    val_loss, val_acc = evaluate(model, val_loader, criterion, get_device())
    return val_acc
